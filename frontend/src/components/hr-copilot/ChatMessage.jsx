              ...
              <TableHead>
                <TableRow>
                  {message.data.columns.map((column) => (
                    <TableCell key={column} sx={{ fontWeight: 700, whiteSpace: "nowrap" }}>{column.replaceAll("_", " ")}</TableCell>
                  ))}
                </TableRow>
              </TableHead>
              <TableBody>
                {message.data.rows.map((row, index) => (
                  <TableRow key={`${message.id}-row-${index}`} hover>
                    {message.data.columns.map((column) => (
                      <TableCell key={column} sx={{ whiteSpace: "nowrap" }}>{row[column] == null ? "—" : String(row[column])}</TableCell>
                    ))}
                  </TableRow>
                ))}
              </TableBody>
